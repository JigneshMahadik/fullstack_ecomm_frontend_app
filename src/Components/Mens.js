...
export function Mens(){
    ...
    async function getProductsBasedOnCollectionCategory(){
        try{
            const resp = await axios.get(`https://fullstack-ecomm-backend-app.onrender.com/getProductsBasedOnCollectionCategory?sub_category=mens&pageNo=${pageNo}&filter=${val}`);
            // console.log("temp is",resp.data.data);
            setMensCollection(resp.data.data);
        }catch(error){
            console.log(error);
        }
    }
