...
export function Kids(){
    ...
    async function getProductsBasedOnCollectionCategory(){
        try{
            const resp = await axios.get(`https://fullstack-ecomm-backend-app.onrender.com/getProductsBasedOnCollectionCategory?sub_category=kids&pageNo=${pageNo}&filter=${val}`);
            // console.log("temp is",resp.data.data);
            setKidsCollection(resp.data.data);
        }catch(error){
            console.log(error);
        }
    }
