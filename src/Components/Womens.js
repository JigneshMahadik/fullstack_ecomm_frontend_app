...
export function Womens(){
    ...
    async function getProductsBasedOnCollectionCategory(){
        try{
            const resp = await axios.get(`https://fullstack-ecomm-backend-app.onrender.com/getProductsBasedOnCollectionCategory?sub_category=womens&pageNo=${pageNo}&filter=${val}`);
            // console.log("temp is",resp.data.data);
            setWomensCollection(resp.data.data);
        }catch(error){
            console.log(error);
        }
    }
